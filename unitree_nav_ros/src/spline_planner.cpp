...
geometry_msgs::Point32 SplinePlanner::runSplineControl(const geometry_msgs::Point32& goal, const double& distance_to_goal)
{
  ...
  // costmap_local_ = costmap_local_vec_[set_goal];
	...
	search_range_ = 4;
	updateCostmap();
	path_safe_set_ = ComputeSafePath(costmap_local_, path_set_4_, map_to_path_4_);
...
bool SplinePlanner::isNeedBack()
{
  isNeedBack_ = false;
  if (path_safe_set_.size() < 60) isNeedBack_ = true;
  return isNeedBack_;
}
...
void SplinePlanner::initLocalCostmap(nav_msgs::OccupancyGrid& Costmap)
{
  ...
  Costmap.info.width  = round((8) / Costmap.info.resolution);
  Costmap.info.height = round((8) / Costmap.info.resolution);

  int costmap_size = Costmap.info.width * Costmap.info.height;
  map_data.resize(costmap_size);

  // COSTMAP中心点的位置
  map_origin.position.x = - 4;
  map_origin.position.y = - 4;
