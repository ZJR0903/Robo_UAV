...
class UnitreeNav // 机器人导航类
{
	public:
		 ...
		 */
		void Manager();
		bool LessSort (DestinationState a, DestinationState b) { return (a.distance_robot_goal > b.distance_robot_goal); }
		void foo(vector<DestinationState> nums) 
		{
			sort(nums.begin(), nums.end(), 
						std::bind(&UnitreeNav::LessSort, 
	...
	private:
		...
		ros::Publisher target_pub_, target_tf_pub_; // 
		ros::Publisher reference_circle_id_pub_; // 
		ros::Publisher red_circle_pub_; // 
		...
		vector<DestinationState> AirSim_global_path_ = vector<DestinationState>(11);
		vector<geometry_msgs::Pose> infer_global_path_;
    ...
    nav_msgs::OccupancyGrid global_costmap_;
      ...
      // zjr 2022.06.22
		...
		void TargetCallBack(const geometry_msgs::Point32::ConstPtr& odom)
		{
			...
			final_pose_map_ = getRealAirSimTarget(AirSim_target_, closed_next_aim_);
		...
		geometry_msgs::Pose getRealAirSimTarget(geometry_msgs::Pose, int);

		inline double hypot3(double x, double y, double z)
		{
			return sqrt(x*x + y*y + z*z);
		}
		geometry_msgs::Pose final_pose_map_;
