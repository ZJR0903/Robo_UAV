...
#target_low, target_up = np.array([5, 230, 100]), np.array([20, 255, 255]) #yellow 上下限

class IMAGE_LEFT:
    def __init__(self,image=None,x=None,y=None,xmax=None,ymax=None):
        print("init")
        self.sub = rospy.Subscriber('/airsim_node/drone_1/front_left/Scene', Image, self.image_callback,queue_size=10)
        self.sub = rospy.Subscriber('/swap', String, self.String_callback, queue_size=10)
        self.target_low, self.target_up = np.array([5, 230, 100]), np.array([20, 255, 255])
    ...
    def image_callback(self, msg): # feedback means actual value.
        ...
        #去除颜色范围外的其余颜色
        mask = cv2.inRange(hsv, self.target_low, self.target_up)
    ...
    def String_callback(self, msg): 
        ...
        self.target_low, self.target_up = np.array([140, 43, 46]), np.array([200, 255, 255])
